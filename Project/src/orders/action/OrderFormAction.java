		...
		if (id == null) {
			...
			if(request.getParameter("type").equals("all")) { //장바구니 전체주문
				...
				for(int i=0;i<cartList.size();i++) {
					...
					probean.setPro_code(cartList.get(i).getPro_code());;
					probean.setPro_name(cartList.get(i).getPro_name());
					probean.setPro_price(cartList.get(i).getPro_price());
					probean.setPro_image(cartList.get(i).getPro_image());
